...
export function Icon({ src, alt, addClassName }) {
    return (
        <Image src={src} alt={alt} height={30} width={30} className={`${addClassName} cursor-pointer`} />
    );
}
...
function MenuList({ isLargeScreen = true, isOpen }) {
    function List({ href, text }) {
        return (
            <li className="font-semibold p-5 pt-3 sm:p-8 lg:pt-0 lg:p-0 lg:cursor-pointer">
                <Link href={href} className={!isLargeScreen ? `hover:text-warm-orange hover:font-semibold hover:underline hover:underline-offset-8 hover:decoration-dashed` : ``}>{text}</Link>
            </li>
        );
    }
    ...
    return (
        <ul className={`${isLargeScreen ? (isOpen ? `bg-black bg-opacity-80 fixed inset-0 flex flex-col mt-16 z-50 h-fit rounded-md sm:flex-row sm:justify-between` : `hidden`) : `flex flex-1 justify-evenly items-center group`}`}>
            <List href={`https://anitaku.pe/anime-list.html`} text={`Anime List`} />
            <List href={`https://anitaku.pe/new-season.html`} text={`New Season`} />
            <List href={`https://anitaku.pe/anime-movies.html`} text={`Movies`} />
            <List href={`https://anitaku.pe/popular.html`} text={`Popular`} />
        </ul>
    );
}
